...
def display_upload_page():
    ...
    def upload_file():
        ...
        x=users.find_one({"username":username_entry.get()})
        password_verifier=x["password"]
        salt=x["salt"]
        hasher = hashlib.blake2b(digest_size=4)
        hasher.update((str(encrypted_file) + password_verifier).encode('utf-8'))
        ...
        if hash_value_server == hash_value:
            ...
            files_upload_temp=file_upload_entry.get()
            data = (username_entry.get(), file_upload_entry.get().split("/")[-1], encrypted_file, hash_value)
            files.insert_one({"username":username_entry.get(),
            "file_name":file_upload_entry.get().split("/")[-1],
            "data":encrypted_file,
            "verfier":hash_value})
            root.destroy()
